...
if choice==1:
	...
	if ch_debug=="y" or ch_debug=="yes":
		rankedText = summarization.summaryGen(prod[ch],domain,debugging=True)
	else:
		rankedText = summarization.summaryGen(prod[ch],domain)	
...
if choice==2:
	...
	if ch_debug=="y" or ch_debug=="yes":
		if ch_token=="y" or ch_token=="yes":
			...
			token=int(raw_input())
			rankedText=summary_file.summaryGen(prod[ch],domain,gram=token,debug=True)
		else:
			rankedText=summary_file.summaryGen(prod[ch],domain,debug=True)
	else:
		if ch_token=="y" or ch_token=="yes":
			print "Enter token size"
			token=int(raw_input())
			rankedText=summary_file.summaryGen(prod[ch],domain,gram=token)
		else:
			rankedText=summary_file.summaryGen(prod[ch],domain)

keys=keywords.extract_keywords(domain,prod[ch])
rankedSummary=""
